try {
  ...
  inquirer.prompt(questions).then(({ username, password }) =>
    staffToken(username, password)
      .then((token) => {
        if (username && password) {
          return fs.readFile(
            `${advisorHtmlPath}/index_tmp.html`,
            'utf-8',
            (err, contents) => {
              if (err) throw err;
              const htmlWithToken = contents
                .toString()
                .replace('{{staffToken}}', token);
              fs.writeFile(
                `${advisorHtmlPath}/index.html`,
                htmlWithToken,
                (err) => {
                  if (err) {
  ...
  // ASYNC/AWAIT won't work on early node versions
  console.warn('Check that your node version is >= 10. re-run npm install.');
  throw err;
}
