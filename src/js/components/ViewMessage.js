...
export class ViewMessage extends React.Component {
  ...
  render() {
    ...
    const { readOnly, content, containerSize, noPadding, modalType } = this.props;
    const messageStatus = messageDetail.status === NEW && readOnly !== true ? READ : messageDetail.status;
      ...
      <div>
        <Container {...getPaddingProps(noPadding)} size={containerSize}>
          <Row {...getRowMarginProps(noPadding)}>
            <Card>
              <SectionHeading heading1={getTitle(messageDetail.status, content)} />

              <MailMessage {...this.props} newMessageStatus={messageStatus} hasAttachment={hasAttachment} justBeenRead={this.state.justBeenRead} basePath={basePath} />
        ...
        {modalType > 0 && <SuccessModal onClick={this.handleCloseSuccessModal} bodyText={getSuccessModalMessage(modalType, content)} okText={content.ok} />}
      </div>
    );
  }
}
