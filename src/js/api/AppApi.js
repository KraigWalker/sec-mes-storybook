...
class AppApi {
	...
	getAccounts(success, error) {
		this.apiUtils.makeRequest({ url: `${this.config.apiBaseUrl}${_getAccountsURL}`, method: 'GET', apiVersion: '0.8.0' }, success, error);
	}
	...
	updateMessageData(requestData, id, status, success, error) {
		...
		this.apiUtils.makeRequest({ url, method: 'PUT', requestData: reqData, apiVersion: '1.2.0' }, success, error);
	}

	fetchAccountSegment(success, error) {
		this.apiUtils.makeRequest({ url: `${this.config.apiBaseUrl2}${_getAccountSegmentEndpoint}`, method: 'GET', apiVersion: '0.8.0' }, success, error);
	}

	fetchCustomerDetails(success, error, id) {
		const updateUrl = `${this.config.apiBaseUrl2}${_getCustomerDetailsEndpoint}`;
		const url = updateUrl.replace('{customer_id}', id);
		this.apiUtils.makeRequest({ url, method: 'GET', apiVersion: '0.8.0' }, success, error);
	}
}
