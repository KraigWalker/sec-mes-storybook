...
class ApiUtils {
  ...
  makeRequest(apiData, onSuccess, onFail) {
    ...
    switch (apiData.method) {
      ...
      case 'POST':
        return axios
          .post(apiData.url, apiData.requestData, {
      ...
      case 'PUT':
        return axios
          .put(apiData.url, apiData.requestData, {
            headers: this.requestHeaders,
          })
          .then((response) => {
            onSuccess(response);
      ...
      case 'DELETE':
        return axios
          .delete(apiData.url, apiData.requestData, {
            headers: this.requestHeaders,
          })
          .then((response) => {
            onSuccess(response.data);
...
export const getStaffHeaders = (session) => ({
  ...
  'x-bpi-trust-created-dt':
    session.scopeCreatedDate || moment().format('YYYY-MM-DDTHH:mm:ss'),
