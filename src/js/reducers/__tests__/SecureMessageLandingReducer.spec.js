...
describe('>>>R E D U C E R --- Test AccessibilityReducer', () => {
    it('+++ reducer for REQUEST_SECURE_MESSAGES', () => {
        ...
        expect(state).toEqual({ messages: [], fetching: true, fetched: false, error: false, successModal: false, activeTab: 'inbox', newMessageError: false, draftError: false, });
    });
    ...
    it('+++ reducer for REQUEST_SECURE_MESSAGES_FAILURE', () => {
        ...
        expect(state).toEqual({ messages: [], fetching: false, fetched: true, error: true, successModal: false, activeTab: 'inbox', newMessageError: false, draftError: false });
    });
    it('+++ reducer for REQUEST_TAB_ACTIVE', () => {
        let state = { messages: [], fetching: false, fetched: false, error: false, successModal: false, activeTab: 'inbox', newMessageError: false, draftError: false };
        state = SecureMessageLandingReducer(state, { type: "REQUEST_TAB_ACTIVE", payload: "draft" });
        expect(state).toEqual({ messages: [], fetching: false, fetched: false, error: false, successModal: false, activeTab: 'draft', newMessageError: false, draftError: false });
    });
    ...
    it('+++ reducer for ERROR_BACK_BUTTON', () => {
        ...
        expect(state).toEqual({ messages: [], fetching: false, fetched: false, error: false, successModal: false, activeTab: 'inbox', newMessageError: false, draftError: false });
    });
